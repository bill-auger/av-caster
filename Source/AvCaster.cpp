...
GstElement*   AvCaster::MixerSink     = nullptr ;
...
*/
  ...
  if (!(Pipeline        = gst_pipeline_new        ("pipeline"                       )) ||
      ...
      !(MixerSink       = gst_element_factory_make("xvimagesink"  , "mixer-sink"    )) ||
...
#endif // CONFIGURE_TEXT_CHAIN
#ifdef CONFIGURE_MIXER_CHAIN
      !gst_bin_add(GST_BIN(Pipeline    ) , MixerBin     ) ||
      !gst_bin_add(GST_BIN(MixerBin    ) , MixerSink    ) ||
#endif // CONFIGURE_MIXER_CHAIN
#ifdef CONFIGURE_MUX_CHAIN
...
#endif // CONFIGURE_CAMERA_CHAIN
#ifdef CONFIGURE_MIXER_CHAIN
  gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(MixerSink    ) , window_handle) ;
#endif // CONFIGURE_MIXER_CHAIN
#ifdef CONFIGURE_SCREENCAP_CHAIN
...
#endif // CONFIGURE_CAMERA_CHAIN
#ifdef CONFIGURE_MIXER_CHAIN
      !gst_video_overlay_set_render_rectangle(GST_VIDEO_OVERLAY(MixerSink    )           ,
                                              GUI::OUTPUT_MONITOR_X    , GUI::MONITORS_Y ,
                                              GUI::MONITORS_W          , GUI::MONITORS_H )  )
#else // CONFIGURE_MIXER_CHAIN
      false)
#endif // CONFIGURE_MIXER_CHAIN
...
/*
  if (!(source     = gst_element_factory_make("ximagesrc"  , "screencap-input" )) ||
      !(capsfilter = gst_element_factory_make("capsfilter" , "screencap-caps"  )) ||
      !(queue      = gst_element_factory_make("queue"      , "screencap-queue" )) ||
      !(caps       = gst_caps_from_string(SCREENCAP_CAPS)                       )  )
...
//   if (!gst_bin_add        (GST_BIN(ScreencapBin) , source)                          ||
//       !gst_bin_add        (GST_BIN(ScreencapBin) , capsfilter      )                          ||
//       !gst_element_link   (source , capsfilter)                                     ||
//       !gst_element_add_pad(ScreencapBin , CreateGhostPad(capsfilter , "src" , "screen-source")))
//   { Error(GUI::SCREENCAP_LINK_ERROR_MSG) ; return false ; }

GstPad* apad ;
GstElement *scale = gst_element_factory_make("videoscale" , "video-scale") ;
GstElement *cvt = gst_element_factory_make("videoconvert" , "video-in-cvt") ;
GstElement *fake = gst_element_factory_make("fakesink" , "fake-sink") ;
if (!gst_bin_add        (GST_BIN(MuxBin) , scale)) { DBG("cvt->ScreencapBin nfg") ; return false ; }
if (!gst_bin_add        (GST_BIN(MuxBin) , cvt)) { DBG("cvt->ScreencapBin nfg") ; return false ; }
if (!gst_bin_add        (GST_BIN(MuxBin) , source)) { DBG("source->ScreencapBin nfg") ; return false ; }
if (!gst_bin_add        (GST_BIN(MuxBin) , capsfilter)) { DBG("capsfilter->ScreencapBin nfg") ; return false ; }
if (!gst_bin_add        (GST_BIN(MuxBin) , queue )) { DBG("queue->ScreencapBin nfg") ; return false ; }
// if (!gst_element_link   (source , ScreencapSink) ) { DBG("source->capsfilter nfg") ; return false ; }
// if (!gst_element_link   (capsfilter , cvt)           ) { DBG("capsfilter-> cvt nfg") ; return false ; }
// if (!gst_element_link   (capsfilter , ScreencapSink)           ) { DBG("capsfilter->ScreencapSink nfg") ; return false ; }
// if (!gst_element_link   (capsfilter , fake)           ) { DBG("capsfilter->ScreencapSink nfg") ; return false ; }
// if (!gst_element_link   (capsfilter , queue)           ) { DBG("capsfilter->ScreencapSink nfg") ; return false ; }

if (!gst_element_link   (source ,    scale)           ) { DBG("source->scale nfg") ; return false ; }
if (!gst_element_link   (scale,      cvt)           ) { DBG("scale->cvt nfg") ; return false ; }
if (!gst_element_link   (cvt ,       capsfilter)           ) { DBG("converter->capsfilter nfg") ; return false ; }
if (!gst_element_link   (capsfilter , queue)           ) { DBG("capsfilter->queue nfg") ; return false ; }

// if (!gst_element_link   (cvt , ScreencapSink)           ) { DBG("capsfilter->ScreencapSink nfg") ; return false ; }

// if (!(apad = CreateGhostPad(capsfilter , "src" , "screen-source"))){ DBG("screenpad nfg") ; return false ; }
// if (!gst_element_add_pad(ScreencapBin , apad)) { DBG("ghost_pad->ScreencapBin nfg") ; return false ; }

if (!gst_element_link   (queue,MuxVideoQueue)           ) { DBG("source->capsfilter nfg") ; return false ; }
*/

  if (!(source     = MakeElement("videotestsrc" , "screencap-input"     )) ||
//       !(converter = MakeElement("videoconvert" , "screencap-converter")) ||
      !(queue      = MakeElement("queue"        , "screencap-queue"     )) ||
      !(capsfilter = MakeElement("capsfilter"   , "screencap-capsfilter")) ||
      !(caps       = gst_caps_from_string(TESTVIDEO_CAPS)                )  )
  { Error(GUI::SCREENCAP_INIT_ERROR_MSG) ; return false ; }
...
//   g_object_set(G_OBJECT(source    ) , "show-pointer" , true            , NULL) ;
  ...
  if (!AddElement  (ScreencapBin , source    )                    ||
      ...
      !AddGhostPad (ScreencapBin , queue , "src" , "screen-source"))
  { Error(GUI::SCREENCAP_LINK_ERROR_MSG) ; return false ; }
...
plugin_id = "videotestsrc" ; caps_str = TESTVIDEO_CAPS ;
  ...
  if (!(source     = MakeElement(CHARSTAR(plugin_id) , "camera-input")) ||
...
g_object_set(source    , "is_live"    , true           , NULL) ;
  ...
  if (!AddElement  (CameraBin , source    )                    ||
      ...
      !LinkElements(source , capsfilter)                       ||
      !LinkElements(capsfilter , queue )                       ||
      !AddGhostPad (CameraBin , queue , "src" , "camera-source"))
...
g_object_set(source , "is_live" , true , NULL) ;
  ...
  if (!AddElement  (AudioBin , source    )                     ||
      ...
      !LinkElements(source     , capsfilter)                   ||
      !LinkElements(capsfilter , queue )                       ||
      !AddGhostPad (AudioBin   , queue , "src" , "audio-source"))
...
// GstElement* null_sink = MixerSink  ;
...
#  ifdef CONFIGURE_COMPOSITING_CHAIN
  if (!AddElement  (TextBin , filesrc  )                    ||
      ...
      !AddGhostPad(TextBin , queue , "src" , "text-source") ||
...
return AddElement(TextBin , filesrc    )                                                   &&
       AddElement(TextBin , subparse   )                                                   &&
       AddElement(TextBin , source     )                                                   &&
       AddElement(TextBin , textconvert)                                                   &&
       AddElemen(TextBin , queue      )                                                   &&
       AddElement(TextBin , null_sink  )                                                   &&
...
// g_object_set(MixerSink  , "async-handling" , TRUE , NULL) ;
g_object_set(capsfilter , "caps"           , caps , NULL) ; gst_caps_unref(caps) ;

  if (!AddElement  (MixerBin , compositor)                           ||
      !AddElement  (MixerBin , capsfilter)                           ||
      !AddElement  (MixerBin , queue     )                           ||
      !LinkElements(compositor , capsfilter)                         ||
      !LinkElements(capsfilter , queue     )                         ||
      !AddGhostPad (MixerBin , compositor , "sink" , "main-sink"   ) ||
      !AddGhostPad (MixerBin , compositor , "sink" , "overlay-sink") ||
      !AddGhostPad (MixerBin , compositor , "sink" , "text-sink"   ) ||
      !AddGhostPad (MixerBin , queue      , "sink" , "mixer-source")  )
...
!AddElement(MuxBin , audio_enc_queue4)       ||
      ...
      !AddGhostPad (MuxBin , video_in_queue , "sink" , "video-sink")  ||
#ifdef CONFIGURE_COMPOSITING_CHAIN
      !LinkElements(MixerBin , MuxBin)                                ||
#else // CONFIGURE_COMPOSITING_CHAIN
#  ifdef CONFIGURE_SCREENCAP_CHAIN
...
!LinkElements   (audio_enc_queue3 , audio_enc_queue4)       ||
      ...
      !AddGhostPad (MuxBin , audio_in_queue , "sink" , "audio-sink")  ||
      !LinkElements(AudioBin , MuxBin) ||
      !AddGhostPad (MuxBin , mux            , "src"  , "mux-source")   )
...
DEBUG_TRACE_CONFIG_OUTPUT
  ...
  if (!(queue = MakeElement("queue"             , "output-queue")) ||
      !(sink  = MakeElement(CHARSTAR(plugin_id) , "output-sink" ))  )
  ...
  if (!AddElement  (OutputBin , queue)                          ||
      ...
      !AddGhostPad (OutputBin , queue , "sink" , "output-sink") ||
...
bool AvCaster::AddGhostPad(GstElement* a_bin          , GstElement* an_element   ,
